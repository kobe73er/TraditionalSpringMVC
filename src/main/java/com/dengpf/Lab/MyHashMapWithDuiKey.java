 ...
 * @param <V>
 */
public class MyHashMapWithDuiKey<K, V> implements MyMap<K, V> {
    ...
    public MyHashMapWithDuiKey() {
        this(DEFAULT_INITAL_CAPACITY, DEFAULT_MAX_LOAD_FACTOR);
    }

    public MyHashMapWithDuiKey(int capacity) {
        this(capacity, DEFAULT_MAX_LOAD_FACTOR);
    }

    public MyHashMapWithDuiKey(int capacity, float loadFactor) {
    ...
    public Set<V> getAll(K key) {
        ...
        if (null != table[hash(key.hashCode())]) {
            for (MyMap.Entry<K, V> item : table[hash(key.hashCode())]) {
...
//            }
            ...
            bucket.add(new MyMap.Entry<K, V>(key, value));
        }
